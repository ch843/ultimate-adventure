...
import { TripMemberSchema } from "@ultimate-adventure/shared-models";
import { TripMemberDAO } from "@ultimate-adventure/backend-utils";

export const tripMemberService = {
  async getTripMember(
    request: GetTripMemberRequest,
  ...
  async getAllTripMembers(
    _request: GetAllTripMembersRequest,
  ): Promise<GetAllTripMembersResponse> {
    const dbTripMembers = await TripMemberDAO.getAllTripMembers();

    // Validate all trip members from DB with Zod
    const tripMembers: TripMember[] = dbTripMembers.map((tm) =>
      TripMemberSchema.parse(tm),
    );

    return { tripMembers };
  },

  async getTripMembersByTripId(
    request: GetTripMembersByTripIdRequest,
  ): Promise<GetTripMembersByTripIdResponse> {
    const dbTripMembers = await TripMemberDAO.getTripMembersByTripId(
      request.tripId,
    );
  ...
  async getTripMembersByMemberId(
    request: GetTripMembersByMemberIdRequest,
  ): Promise<GetTripMembersByMemberIdResponse> {
    const dbTripMembers = await TripMemberDAO.getTripMembersByMemberId(
      request.memberId,
    );
  ...
  async createTripMember(
    request: CreateTripMemberRequest,
  ): Promise<CreateTripMemberResponse> {
  ...
  ): Promise<UpdateTripMemberResponse> {
  ...
  async deleteTripMember(
    request: DeleteTripMemberRequest,
