...
export const clubMemberService = {
  async getClubMember(request: GetClubMemberRequest): Promise<GetClubMemberResponse> {
  ...
  async getAllClubMembers(_request: GetAllClubMembersRequest): Promise<GetAllClubMembersResponse> {
    const dbMembers = await ClubMemberDAO.getAllClubMembers();

    // Validate all members from DB with Zod
    const members: ClubMember[] = dbMembers.map(member => ClubMemberSchema.parse(member));

    return { members };
  },

  async createClubMember(request: CreateClubMemberRequest): Promise<CreateClubMemberResponse> {
  ...
  async updateClubMember(request: UpdateClubMemberRequest): Promise<UpdateClubMemberResponse> {
    await ClubMemberDAO.updateClubMember(request.id, request.data);

    return { success: true };
  },

  async deleteClubMember(request: DeleteClubMemberRequest): Promise<DeleteClubMemberResponse> {
