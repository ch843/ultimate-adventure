...
class _TripMemberDAO extends BaseDAO<"Trip Members"> {
  ...
  public async createTripMember(
    ...
    return this.create<Tables<"Trip Members">, typeof tripMemberData>(
      tripMemberData,
    );
  }
  ...
  public async getTripMembersByTripId(
    tripId: number,
  ): Promise<Tables<"Trip Members">[]> {
    ...
    if (error) {
      throw new Error(
        `Failed to fetch trip members for trip ${tripId}: ${error.message}`,
      );
    }
  ...
  public async getTripMembersByMemberId(
    memberId: number,
  ): Promise<Tables<"Trip Members">[]> {
    const { data, error } = await this.client
      .from(this.tableName)
    ...
    if (error) {
      throw new Error(
        `Failed to fetch trip members for member ${memberId}: ${error.message}`,
      );
    }
