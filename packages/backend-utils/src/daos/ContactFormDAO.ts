...
class _ContactFormDAO {
  ...
  public async createContactForm(formData: ContactFormInsert): Promise<Tables<'Contact Form Info'>> {
  ...
  public async getContactForm(id: number): Promise<Tables<'Contact Form Info'>> {
    const { data, error } = await this.client
      .from('Contact Form Info')
      .select('*')
      .eq('id', id)
